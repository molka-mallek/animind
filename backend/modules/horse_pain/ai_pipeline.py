...
class _BiLSTMModel:
    ...
    def __init__(self, path, device):
        ...
        if isinstance(checkpoint, dict) and "model_state_dict" in checkpoint:
            ...
            # Assume it's a full nn.Module saved with torch.save(model)
        ...
        # Try to infer input_size from state dict
...
def _extract_features_from_video(video_path: str, seq_len: int = 30) -> np.ndarray:
    """
    Extract per-frame optical-flow magnitude + pose proxy features from a video.
    ...
    # Sample evenly to get seq_len frames
    indices = np.linspace(0, len(frames) - 2, seq_len, dtype=int)

    feature_rows = []
    prev = frames[0]
    for idx in indices:
        curr = frames[min(idx + 1, len(frames) - 1)]
        ...
        row = []
        # Optical flow stats
        row.append(float(mag.mean()))
        row.append(float(mag.std()))
        row.append(float(mag.max()))
        row.append(float(ang.mean()))
        row.append(float(ang.std()))

        # Pad or trim to match n_features
        while len(row) < _n_features:
            row.append(0.0)
        row = row[:_n_features]

        feature_rows.append(row)
        prev = frames[min(idx + 1, len(frames) - 1)]
